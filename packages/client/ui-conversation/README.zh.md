...
Assistant 正文使用紧凑的桌面端排版。选中 Assistant 回复中的文字时，选区旁会出现小型批注编辑器；确认后，Markdown 引用和本地化批注标签会追加到对应会话的现有输入草稿，并聚焦输入框，是否发送仍由用户决定。按 Escape、点击外部、滚动或调整窗口大小会关闭编辑器且不改动草稿。
