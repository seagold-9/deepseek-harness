...
Assistant 正文使用紧凑的桌面端排版。选中 Assistant 回复中的文字时，浏览器选区会保留，并在选择终点旁出现操作菜单，因此复制与批注互不干扰，长选区也不会让操作入口远离鼠标。只要拖选从回复正文内部开始，即使鼠标在回复边缘之外松开，也会识别该选区。选择**添加到对话**后才会在同一终点打开可选评论编辑框；确认后，Markdown 引用和已填写的批注会追加到对应会话的现有输入草稿并聚焦输入框，是否发送仍由用户决定。原回复不会保留高亮或引用编号。按 Escape、点击外部、滚动或调整窗口大小会关闭尚未确认的浮层且不改动草稿。
